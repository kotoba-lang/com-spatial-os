(ns spatial-os.main
  "Kotodama WASM entrypoint for the Spatial_os clean-room actor (L4) — Clojure port.
  ...
  facts (`spatial_os.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "spatial_os")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"spatial_os.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "spatial_os-compat" :tier tier :entities entities} 200])
...
(defn start! [] :spatial_os-compat/ready)
